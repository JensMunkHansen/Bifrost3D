...
public:
    ...
    inline void button_tapped(Button button, bool pressed) {
        unsigned char button_ID = static_cast<unsigned char>(button);
    ...
    inline bool is_pressed(Button button) const { return m_button_states[static_cast<unsigned char>(button)].is_pressed && !m_button_states[static_cast<unsigned char>(button)].is_consumed; }
    inline bool is_released(Button button) const { return !is_pressed(button); }
    inline unsigned int halftaps(Button button) const { return m_button_states[static_cast<unsigned char>(button)].halftaps; }

    inline bool was_pressed(Button button) const {
        const ButtonState state = m_button_states[static_cast<unsigned char>(button)];
    ...
    inline bool is_consumed(Button button) const { return m_button_states[static_cast<unsigned char>(button)].is_consumed; }
    inline void consume_button_event(Button button) { m_button_states[static_cast<unsigned char>(button)].is_consumed = true; }
